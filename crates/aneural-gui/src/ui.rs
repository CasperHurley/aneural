    ...
    egui::Panel::bottom("spores")
        ...
        .show(ctx, |ui| {
            ui.horizontal(|ui| {
                ui.heading("Spores");
                ui.label(
                    egui::RichText::new("derived from your files — nothing to maintain")
                        .weak()
                        .small(),
                );
            });
            let spore_kinds: Vec<String> = ws
                .node_types
                .iter()
                .filter(|d| d.provider.starts_with("spore:"))
                .map(|d| d.kind.clone())
                .collect();
            let mut select_next: Option<NodeId> = None;
            ui.horizontal_top(|ui| {
                for kind in spore_kinds {
                    let style = ws.style(&kind);
                    let (total, _) = kind_counts.get(&kind).copied().unwrap_or((0, 0));
                    ui.vertical(|ui| {
                        ui.set_min_width(220.0);
                        ui.label(
                            egui::RichText::new(format!("{} · {total}", style.label))
                                .color(theme::egui_color(style.color))
                                .strong(),
                        );
                        egui::ScrollArea::vertical()
                            ...
                            .max_height(120.0)
                            .show(ui, |ui| {
                                let mut rows: Vec<&GraphNode> = nodes
                                    .iter()
                                ...
                                rows.sort_by(|a, b| {
                                    a.props
                                        .get("file")
                                        .and_then(|f| f.as_str())
                                        .cmp(&b.props.get("file").and_then(|f| f.as_str()))
                                        .then(a.label.cmp(&b.label))
                                });
                                for n in rows.into_iter().take(50) {
                                    let file = n
                                        .props
                                        .get("file")
                                        .and_then(|f| f.as_str())
                                    ...
                                    let line = n
                                        ...
                                        .unwrap_or_default();
                                    ui.horizontal(|ui| {
                                        if ui.link(egui::RichText::new(&n.label).small()).clicked()
                                        {
                                            select_next = Some(n.id.clone());
                                        }
                                        ui.label(
