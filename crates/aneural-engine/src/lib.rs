...
impl Engine {
    ...
    pub fn spores(&self) -> Vec<SporeInfo> {
        self.spores.iter().map(Spore::info).collect()
    }
    ...
    fn process_file(&mut self, entry: &Entry, force: bool) -> Result<Processed> {
        ...
        if self.spores.iter().any(|s| s.enabled && s.applies_to(rel)) {
            let h = spores::harvest_file(&self.spores, &self.md_index, rel, &bytes);
    ...
    pub fn harvest_spore(
        ...
        let Some(idx) = self.spores.iter().position(|s| s.manifest.name == name) else {
        ...
        for rec in self.store.all_files()? {
            ...
            if let Processed::Indexed(d) = self.process_file(&entry, true)? {
                ...
                stats.nodes += d
                    .nodes
                    .iter()
                    .filter(|n| n.source == aneural_core::kinds::Source::spore(name))
                    .count() as u64;
                stats.edges += d
                    .edges
                    .iter()
                    .filter(|e| e.source == aneural_core::kinds::Source::spore(name))
                    .count() as u64;
                self.emit(d, sink);
            }
        }
